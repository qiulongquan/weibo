...
def _encode_params(**kw):
    ...
    return parse.urlencode(kw).encode('utf-8')
...
def _encode_multipart(**kw):
    ...
    for k, v in kw.items():
        ...
        if hasattr(v, 'read'):
            ...
            content = v.read()
            data.append(
                'Content-Disposition: form-data; name="%s"; filename="hidden"' % k)
...
def _http_call(the_url, method, authorization, **kw):
    ...
    try:
        ...
        if hasattr(r, 'error_code'):
            raise APIError(r.error_code, r.get(
                'error', ''), r.get('request', ''))
        return r
...
class APIClient(object):
    ...
    def parse_signed_request(self, signed_request):
        ...
        expected_sig = hmac.new(
            self.client_secret, enc_payload, hashlib.sha256).digest()
        if expected_sig == sig:
    ...
    def get_authorize_url(self, redirect_uri=None, **kw):
        ...
        if not redirect:
            raise APIError(
                '21305', 'Parameter absent: redirect_uri', 'OAuth2 request')
