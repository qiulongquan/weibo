...
if __name__ == '__main__':
    ...
    code = 'c05cef6a331a44a0e8c1dc9a570f311c'
    ...
    # 通过http方法来调用API接口，并取得返回值
    resp_text = _http('GET', 'https://api.weibo.com/2/users/show.json',
                      access_token=access_token,uid=uid)
    r = _parse_json(resp_text)
    # JsonDict型的数据直接用 .key 来提取值
    print ("id ",r.id)
    print ("name ",r.name.encode("utf-8"))
    print ("location ", r.location.encode("utf-8"))
    print ("description ", r.description)
    print ("image ", r.profile_image_url)
    print ("gender ", r.gender)
    print ("followers_count ", r.followers_count)
    print ("friends_count ", r.friends_count)
    print ("pagefriends_count ", r.pagefriends_count)
    print ("statuses_count ", r.statuses_count)
    # 也可以用  .get 方法来获取值  r.get('remind_in', None)
    # remind_in = r.get('remind_in', None)
